/* Generate ImageJ/Fiji stitching configuration file directory of DSX images
	...
	v230202-6: Adds more options including just stitching a limited number of initial tiles (useful for checking overlap).
	...
	v230811-12: Updated to match "new-plugin" version. Removed irrelevant options.
	v230814: Removed redundant message lines. Updated safeSaveAndClose function.
	v230822: Corrected selectImage to selectWindow. F1: Updated indexOfArray functions. F2: Updated safeSaveAndClose
	v231020: Minor dialog.addCheckboxGroup fix. F1: Updated sensibleUnits function.
	v240222: Improved grammar, added toolbar flashes.
	v240227: Saved path of registered position file to prefs. F1: Updated safeSaveAndClose
	*/
	macroL = "IJStitch_DSX_Mosaic_From_Selected_Image(old_plugin)_v240227-f1.ijm";
	...
	Dialog.create("DSX IJ Stitch Options \(" + macroL + "\)");
		...
		blendingOptions = newArray("Linear Blending", "Average", "Median", "Max. Intensity", "Min. Intensity", "None");
		...
		if (i==0){
			...
			if (toLowerCase(ext)=="dsx"){
				...
				if (useStagePosn){
					...
					tileXs[i] = 0;
					tileYs[i] = 0;
					tileConfig += "" + dir + filteredDSXMapFiles[i] + "\; \; \(0,0\)\n";
				}
			}
			...
			if (useStagePosn)	reportLog += ", stage positions used for tile positions";
				...
				tileConfig += "" + dir + filteredDSXMapFiles[i] + "\; \; \(" + d2s(tileXs[i],1) + ", " +  d2s(tileYs[i],1) + "\)\n";
		}
		if (!useStagePosn)
			tileConfig += "" + dir + filteredDSXMapFiles[i] + "\; \; \(" + d2s((tileXs[i]-1)*overlapAdvX,0) + ", " +  d2s((tileYs[i]-1)*overlapAdvY,0) + "\)\n";
	}
	...
	run("Close");
	stitchOptions = "layout=[&configPath]";
	if (computeOverlap) stitchOptions += " compute_overlap";
	stitchOptions += " channels_for_registration=[Red, Green and Blue] rgb_order=rgb rgb_order=rgb fusion_method=[&blendingOption] fusion=&fusionVal regression=&regressionVal max/avg=&maxAvgVal absolute=&absVal";
	showStatus("Starting stitch of " + tileN + " tiles");
	if (!is("Batch Mode")) setBatchMode("hide");
	showStatus("Stitching image set . . . ");
	if (diagnostics) IJ.log("Stitch options used:\n" + stitchOptions);
	run("Stitch Collection of Images", stitchOptions);
	if(diagnostics) IJ.log("Stitch commands:\n" + stitchOptions);
	nTitle = prefixF + "_IJ-Stitch";
	if (!startsWith(blendingOption, "None")){
		waitForOpenWindow("Stitched Image", 500, 1000, 1000 * tileN);
	...
	if (checkForMissingTiles){
		regFilePath = configPath + ".registered";
		...
		if (mTileN>0) {
			...
			IJ.log("Rerun config path \(Missing tiles only\): " + reConfigPath);
		}
